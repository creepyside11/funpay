...
13. `BIND_TO_PRE_DELIVERY` и `BIND_TO_POST_DELIVERY` вызываются вокруг встроенной автовыдачи.
...
### 2.1. Публикация в общем каталоге

После локальной установки пользователь может открыть:

`Плагины → Мои плагины → <название> → Опубликовать в каталоге`

Бот запрашивает отдельное подробное описание длиной 40–2000 символов. Оно должно объяснять:

- назначение и основной сценарий использования;
- добавляемые команды и кнопки;
- порядок настройки;
- требуемые разрешения, внешние сервисы и зависимости;
- важные ограничения и потенциально опасные действия.

В каталог копируются текущие метаданные и полный исходник установленного файла. Публикация общая
для всех пользователей одного экземпляра бота. Любой пользователь может открыть карточку, прочитать
описание, скачать `.py` и после отдельного подтверждения установить его себе. Автор публикации
определяется по Telegram ID; отображаемое имя — username или полное имя на момент публикации.

Один UUID может принадлежать только одной публикации. Автор может обновить свою карточку: при
подтверждении бот заново копирует актуальную версию, метаданные и исходник установленного плагина.
Чужую или официальную публикацию перезаписать нельзя. При снятии публикации она исчезает из каталога,
но уже установленные у других пользователей копии не удаляются.

Каталог сообщества не является песочницей и не гарантирует безопасность. Перед установкой бот даёт
скачать исходник и предупреждает, что код сможет обращаться к FunPay, golden_key, сети, базе и
переменным окружения. Не включайте в описание или исходник секреты, персональные данные и токены.
...
| `BIND_TO_PRE_DELIVERY` | перед отправкой шаблона автовыдачи; аргументы `(cardinal, event)` |
| `BIND_TO_POST_DELIVERY` | после отправки или ошибки автовыдачи; аргументы `(cardinal, event)` |

Для delivery-хуков используется тот же `events.NewOrderEvent`, дополненный полями:

| Поле | Тип | Значение |
|---|---|---|
| `event.order` | `OrderShortcut` | новый заказ FunPay |
| `event.delivery_rule_id` | `int` | ID правила в PostgreSQL |
| `event.delivered` | `bool` | подтверждена ли отправка товара в чат |
| `event.delivery_text` | `str \| None` | итоговый отправленный текст, включая подставленный товар |
| `event.goods_delivered` | `int` | число выданных единиц |
| `event.goods_left` | `int` | остаток строк товара после попытки |
| `event.error` | `int` | `0` при нормальной обработке, `1` при ошибке |
| `event.error_text` | `str \| None` | причина ошибки или нехватки товара |

PRE-хук вызывается после атомарного резервирования строк товара, но до отправки сообщения. POST-хук
вызывается и при успехе, и при нехватке товара, и при ошибке отправки. Если отправка не удалась,
зарезервированные строки возвращаются в начало запаса до POST-хука. Не изменяйте перечисленные поля
для управления встроенной логикой: они предназначены для наблюдения и интеграций.
...
Плагин не должен самостоятельно запрашивать номер, код или пароль 2FA. Сообщения с этими
значениями бот удаляет по возможности; одноразовый код не сохраняется. После входа номер,
Telethon `StringSession` и пароль 2FA шифруются тем же `APP_SECRET`, который защищает golden_key.
Можно последовательно добавить несколько Telegram-аккаунтов одному плагину; повторный вход в тот же
аккаунт обновляет его сессию. Записи изолированы по владельцу бота, UUID плагина и Telegram user ID. Зашифрованный пароль
может использоваться доверенной автоматизацией плагина без повторного запроса, например для
передачи владельца канала. При удалении плагина или ручном отключении сессия и пароль удаляются
каскадно.
...
Для плагина с несколькими аккаунтами используйте `cardinal.telethon.get_clients(UUID)`. Метод
возвращает список всех подключённых клиентов; `get_client(UUID)` сохранён для обратной совместимости
и возвращает первый из них. Долгоживущие объекты, созданные конкретным аккаунтом (например, каналы),
должны хранить ID соответствующей сессии и всегда обслуживаться тем же клиентом.
