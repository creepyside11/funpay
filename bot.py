...
logger.info(f"Всего переменных окружения: {len(all_env_keys)}")
logger.info(f"Список переменных: {all_env_keys}")

token_candidates = [k for k in all_env_keys if any(x in k.upper() for x in ['TOKEN', 'BOT', 'KEY'])]
db_candidates = [k for k in all_env_keys if any(x in k.upper() for x in ['DATABASE', 'DB', 'POSTGRES', 'PG'])]
logger.info(f"Кандидаты на токен: {token_candidates}")
logger.info(f"Кандидаты на БД: {db_candidates}")

BOT_TOKEN = os.environ.get('BOT_TOKEN', '').strip()
DATABASE_URL = os.environ.get('DATABASE_URL', '').strip()

logger.info(f"BOT_TOKEN: {'✅ Найдено (' + str(len(BOT_TOKEN)) + ' символов)' if BOT_TOKEN else '❌ Пусто'}")
logger.info(f"DATABASE_URL: {'✅ Найдено' if DATABASE_URL else '❌ Пусто'}")
logger.info("=" * 60)
...
if not BOT_TOKEN or not DATABASE_URL:
    ...
    logger.error("📌 Проверьте в панели Bothost: переменные должны называться ТОЧНО BOT_TOKEN и DATABASE_URL")
    logger.error("📌 После сохранения переменных нажмите 'Перезапустить'")
    sys.exit(1)
...
from sqlalchemy import select, update, func, Boolean, Integer, String, Float, DateTime, Text
...
def make_account(key: str, proxy: Optional[str]) -> FunPayAPI.Account:
    ...
    acc.requests_timeout = 60
    return acc


def reply_kb(chat_id, name):
...
async def status(m: types.Message):
    ...
    await m.answer(
        f"**Статус:**\n• Ключ: {'✅' if u.golden_key else '❌'}\n"
        f"• Прокси: {'✅' if u.proxy else '❌'}\n• Слушатель: {lst}\n• Ошибок за 24ч: {errors}",
        parse_mode="Markdown"
    )
...
async def errors_cmd(m: types.Message):
    uid = m.from_user.id
    async with db() as s:
        r = await s.execute(select(ErrorLog).where(ErrorLog.user_id == uid)
...
async def pr_cb(c: types.CallbackQuery, state: FSMContext):
    await c.message.answer("🔧 Введи прокси:\n`http://user:pass@ip:port`\n`socks5://user:pass@ip:port`\n\n/skip - пропустить",
                           parse_mode="Markdown")
...
async def key_cb(c: types.CallbackQuery, state: FSMContext):
    await c.message.answer("🔑 Введи Golden Key:")
...
async def key_msg(m: types.Message, state: FSMContext):
    ...
    async with db() as s:
        ...
        try:
            ...
            await s.commit()
            await state.clear()
            await m.answer(f"✅ Ключ принят! Пользователь: `{acc.username}`\nНажми 🔄 Рестарт",
                           reply_markup=main_kb(), parse_mode="Markdown")
        except Exception as e:
            await log_error(uid, f"Key: {e}")
...
async def bal_cb(c: types.CallbackQuery):
    ...
    if not u or not u.golden_key:
        return await c.answer("❌ Настрой ключ!", show_alert=True)
    await c.answer("⏳")
    try:
        acc = make_account(u.golden_key, u.proxy)
        acc.get()
        txt = "**💰 Баланс:**\n\n" + "\n".join(f"• {k}: {v}" for k, v in acc.balance.items())
...
async def stats_cb(c: types.CallbackQuery):
    ...
    async with db() as s:
        ...
        mc, ms = mo.one()
        a = await s.execute(select(func.count(Order.id), func.sum(Order.price))
                            .where(Order.user_id == uid))
        ac, asum = a.one()
    await c.message.answer(
        f"**📊 Статистика:**\n\n"
        f"• Сегодня: {ts or 0}₽ ({tc or 0} зак.)\n• Неделя: {ws or 0}₽ ({wc or 0} зак.)\n"
        f"• Месяц: {ms or 0}₽ ({mc or 0} зак.)\n• Всего: {asum or 0}₽ ({ac or 0} зак.)",
        parse_mode="Markdown"
    )
...
async def r_cb(c: types.CallbackQuery, state: FSMContext):
    ...
    await state.update_data(cid=parts[1], cname="_".join(parts[2:]))
    await state.set_state(ReplyState.waiting)
    await c.message.answer(f"💬 Ответ в чат (или /cancel):")
...
async def listener_loop(uid: int, bot: Bot):
    ...
    while restarts < max_restarts:
        try:
            ...
            await log_error(uid, f"Listener: {e}")
            try:
                await bot.send_message(uid, f"⚠️ Рестарт #{restarts} через {delay}с\n`{e}`", parse_mode="Markdown")
            except:
                pass
            await asyncio.sleep(delay)
    try:
        await bot.send_message(uid, "❌ Слушатель остановлен. Нажми /restart")
    except:
        pass


async def handle_order(ev, uid, acc, bot):
    o = ev.order
    async with db() as s:
        ...
        if u and u.notify_orders:
            try:
                await bot.send_message(uid, f"📦 **Новый заказ!**\n• `#{o.id}`\n• {o.buyer_username}\n"
                                            f"• {o.description[:100]}\n• {o.price}₽",
                                       parse_mode="Markdown",
...
async def handle_msg(ev, uid, acc, bot):
        ...
        if u and u.notify_messages:
            try:
                await bot.send_message(uid, f"💬 **Сообщение!**\n• От: {m.author}\n• {m.chat_name}\n"
                                            f"• {m.text[:200]}", parse_mode="Markdown",
...
async def handle_last_msg(ev, uid, bot):
    ...
    async with db() as s:
        ...
        if u and u.notify_messages:
            try:
                await bot.send_message(uid, f"💬 От {ev.chat.name}: {str(ev.chat)[:200]}",
                                       reply_markup=reply_kb(ev.chat.id, ev.chat.name))
            except:
                pass
