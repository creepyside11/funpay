...
READY_PLUGINS = (
    ...
    ReadyPluginSpec(
        ...
        "1.2.0",
        "Склад Telegram-каналов для нескольких лотов и аккаунтов",
        "Распределяет подготовку между несколькими Telethon-аккаунтами и поддерживает отдельный "
        "резерв публичных каналов для каждого привязанного лота. "
        "раз в пять минут проверяет реальное число подписчиков и запрашивает refill при просадке. "
        "После покупки мгновенно резервирует готовый канал, подтверждает username покупателя "
        "и безопасно передаёт ему владельца.",
...
class Database:
            ...
            BEGIN
                IF EXISTS (
                    SELECT 1 FROM pg_constraint
                     WHERE conname='funpay_plugin_telethon_sessions_pkey'
                       AND conrelid='funpay_plugin_telethon_sessions'::regclass
                       AND cardinality(conkey)=2
                ) THEN
                    ALTER TABLE funpay_plugin_telethon_sessions
                        DROP CONSTRAINT funpay_plugin_telethon_sessions_pkey;
                END IF;
                IF NOT EXISTS (
                    SELECT 1 FROM pg_constraint
                     WHERE conname='funpay_plugin_telethon_sessions_pkey'
                    ...
                    ALTER TABLE funpay_plugin_telethon_sessions
                        ADD CONSTRAINT funpay_plugin_telethon_sessions_pkey PRIMARY KEY (id);
                END IF;
            END $$;
            CREATE UNIQUE INDEX IF NOT EXISTS funpay_plugin_telethon_account_uidx
                ON funpay_plugin_telethon_sessions
                    (telegram_id, plugin_uuid, telegram_user_id);
    ...
    async def get_plugin_telethon_session(
        self, telegram_id: int, uuid: str, session_id: int | None = None
    ) -> asyncpg.Record | None:
        return await self.fetchrow(
            """
    ...
    async def get_plugin_telethon_sessions(
    ...
    async def get_plugin_telethon_session_by_id(
        self, telegram_id: int, session_id: int
    ) -> asyncpg.Record | None:
        return await self.fetchrow(
            """SELECT * FROM funpay_plugin_telethon_sessions
    ...
    async def save_plugin_telethon_session(
        ...
        return await self.fetchrow(
            ...
            ON CONFLICT (telegram_id, plugin_uuid, telegram_user_id) DO UPDATE
...
"""
        ...
        pages = {
            ...
            "telethon": (
                ...
                "сам собирать эти значения. Код не сохраняется; номер, StringSession и 2FA "
                "шифруются через APP_SECRET отдельно для пользователя и UUID плагина.\n\n"
    ...
    async def show_plugin_telethon(target: Message, user_id: int, uuid: str) -> None:
        ...
        if not plugin:
            ...
            return
        sessions = await db.get_plugin_telethon_sessions(user_id, uuid)
        if sessions:
            text_lines = [
            ...
            for index, row in enumerate(sessions, start=1):
                ...
                client = manager.telethon.get_client_by_session(
                    user_id, uuid, int(row["id"])
                )
                status = "🟢" if client and client.is_connected() else "🟡"
                password_status = "2FA сохранён" if row["password_enc"] else "без 2FA"
                text_lines.extend([
                    "",
                    f"{status} <b>{index}. {html.escape(username)}</b>",
                    f"Телефон: <code>{html.escape(phone)}</code> · {password_status}",
                ])
                rows.append([(
                    f"🚪 Отключить {username}",
                    f"{PLUGIN_TELETHON_DISCONNECT_ASK_PREFIX}{row['id']}",
                )])
            text_lines.extend([
                "",
                "Все сессии и пароли 2FA хранятся зашифрованно; коды входа не сохраняются.",
            ])
            text = "\n".join(text_lines)
            rows.insert(0, [("➕ Добавить Telegram-аккаунт", f"plugin_telethon_start:{uuid}")])
            ...
            text = (
                ...
                "Код и пароль удаляются из чата. После входа сессия и пароль 2FA "
                "сохраняются только в зашифрованном виде."
            )
            if not configured:
                text += (
    ...
    async def plugin_telethon_code(message: Message, state: FSMContext) -> None:
        ...
        try:
            ...
            await message.answer(
                ...
                "Сообщение будет удалено, а пароль сохранится зашифрованно для автоматических "
                "операций плагина."
            )
            return
    ...
    async def plugin_telethon_disconnect_ask(callback: CallbackQuery) -> None:
        ...
        await callback.message.answer(
            ...
            reply_markup=keyboard([
                [(
                    "Да, отключить",
                    f"{PLUGIN_TELETHON_DISCONNECT_DO_PREFIX}{session_id}",
                )],
    ...
    async def plugin_telethon_disconnect_do(callback: CallbackQuery) -> None:
        ...
        plugin_runtime = manager.plugins.runtimes.get(callback.from_user.id)
        client = manager.telethon.get_client_by_session(
            callback.from_user.id, uuid, session_id
        )
