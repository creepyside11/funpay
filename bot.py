...
READY_PLUGINS = (
    ...
    ReadyPluginSpec(
        ...
        "1.1.0",
        "Склад заранее подготовленных Telegram-каналов",
        "Заранее создаёт через Telethon и накручивает настраиваемый резерв публичных каналов, "
...
"""
    ...
    async def show_plugin_telethon(target: Message, user_id: int, uuid: str) -> None:
        ...
        if row:
            ...
            password_status = (
                "🔐 2FA сохранён зашифрованно"
                if row["password_enc"]
                else "2FA не сохранён"
            )
            text = (
                ...
                f"Статус: {status}\n"
                f"Пароль: <b>{password_status}</b>\n\n"
                "Сессия и пароль 2FA зашифрованы. Код входа не сохраняется."
            )
            rows = [
